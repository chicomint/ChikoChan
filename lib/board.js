...
class BoardService {
  ...
  validateText(input, image, isThread) {
    ...
    if (!comment && !fortune && !image) throw httpError(400, 'A comment or image is required.');
  ...
  latestImages(limit = 24, data = this.getData()) {
    ...
    for (const entry of this.latestPosts(Number.MAX_SAFE_INTEGER, data)) {
      ...
      if (!post.image || post.imageDeleted) continue;
      const imagePath = this.uploads.pathForPost(post);
      if (!imagePath || !this.uploads.inspectServedFile(path.basename(post.image))) continue;

      const imageKey = post.sha256 || post.image;
