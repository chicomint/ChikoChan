...
<body class="site-home">
  ...
  imageHTML(post, isReply = false) {
    ...
    const imageDimensions = post.width && post.height
      ? ` width="${Number(post.width)}" height="${Number(post.height)}"`
      : '';
    ...
    return `
      ...
      <div class="image-box file${isReply ? ' image-box-reply' : ''}${spoilerClass}">
        <a class="fileThumb" id="f${post.id}" href="${source}" target="_blank" rel="noopener">
          <img class="post-img" src="${source}" alt="${post.spoiler ? 'spoilered image' : 'attached image'}"${imageDimensions} loading="lazy" decoding="async" data-expand-image>
        </a>
      </div>`;
  }
...
<body>
  ...
  home(data, boards, siteStats, latestPosts = [], latestImages = []) {
    ...
    const latestImageItems = latestImages.map(entry => {
      const url = `${this.threadPath(entry.board, entry.threadId)}#p${entry.post.id}`;
      const source = `/${escapeHTML(entry.post.image)}`;
      const dimensions = entry.post.width && entry.post.height
        ? ` width="${Number(entry.post.width)}" height="${Number(entry.post.height)}"`
        : '';
      const spoilerClass = entry.post.spoiler ? ' latest-image-spoiler' : '';
      const label = `/${entry.board.uri}/ post No.${entry.post.id}`;
      return `<a class="latest-image-link" href="${url}" title="${escapeHTML(label)}"><img class="latest-image-thumb${spoilerClass}" src="${source}" alt="Image from ${escapeHTML(label)}"${dimensions} loading="lazy" decoding="async"></a>`;
    }).join('');
