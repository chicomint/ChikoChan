...
class MaintenanceRunner {
  constructor(config, store, service, options = {}) {
    ...
    this.quarantineRetentionMs = (Number(config.lifecycle?.quarantineRetentionHours) || 24) * 60 * 60 * 1000;
    this.store = store;
  ...
  async executeCycle(now = Date.now()) {
    ...
    try {
      ...
      const quarantinedFiles = await this.service.uploads.cleanupQuarantine(now, this.quarantineRetentionMs);
      const result = {
        status: 'completed',
        startedAt: now,
        finishedAt: Date.now(),
        ...
        purgedQuarantineFiles: quarantinedFiles
      };
      this.lastResult = result;
      return result;
    } finally {
