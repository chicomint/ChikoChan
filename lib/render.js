...
const { escapeHTML, formatBytes, previewText } = require('./utils');
...
class Renderer {
  ...
  <header class="site-header">
    <div class="site-top-bar"><a href="/">[${this.t('nav.home')}]</a>${this.themeToggle()}</div>
    ${banner}
    ${announcement}
  </header>`;
  }
...
<body class="site-home">
  ...
  captchaFormAttribute() {
    return this.config.antiAbuse.turnstile.enabled ? ' data-turnstile-form' : '';
  }

  captchaHTML() {
    if (!this.config.antiAbuse.turnstile.enabled) return '';
    return `<div class="turnstile-widget" data-turnstile-widget data-sitekey="${escapeHTML(this.config.antiAbuse.turnstile.siteKey)}" data-action="post"><span class="captcha-status">${this.t('captcha.loads_on_use')}</span></div><noscript><p class="captcha-noscript">${this.t('captcha.javascript_required')}</p></noscript>`;
  }
  ...
  postHeader(post, thread, board, isOp, postIndex) {
    ...
    return `
      <div class="${isOp ? 'thread-header' : 'reply-header'} postInfo desktop" id="pi${post.id}">
        ...
        <time class="date-time dateTime" datetime="${new Date(post.createdAt).toISOString()}">${this.formatDate(post.createdAt)}</time>
  ...
  replyHTML(reply, thread, postIndex, board) {
    ...
    <div class="reply-container postContainer replyContainer" id="pc${reply.id}" data-no="${reply.id}">
      ...
      <div class="reply post" id="p${reply.id}">
        ...
        <blockquote class="comment postMessage" id="m${reply.id}">${message}</blockquote>
      </div>
    </div>`;
  }
  ...
  replyForm(thread, board, options = {}) {
    ...
    const captcha = this.captchaHTML();
    return `
    <details class="reply-form-container" id="reply-form-${thread.id}"${open}>
      ...
      <form class="reply-form" action="/${board.uri}/post" method="POST" enctype="multipart/form-data"${this.captchaFormAttribute()}>
    ...
    <article class="thread" id="t${thread.id}" data-board="${escapeHTML(board.uri)}" data-thread-id="${thread.id}">
      <div class="postContainer opContainer" id="pc${thread.id}" data-no="${thread.id}">
        <div class="post op" id="p${thread.id}">
          ...
          <blockquote class="comment op-comment postMessage" id="m${thread.id}">${message}</blockquote>
        </div>
      </div>
  ...
  <div class="post-form-wrapper">
    <form id="post-form" name="post" action="/${board.uri}/post" method="POST" enctype="multipart/form-data"${this.captchaFormAttribute()}>
      <input type="hidden" name="board" value="${escapeHTML(board.uri)}">
      <input type="hidden" name="mode" value="regist">
  ...
  catalogItem(thread, board) {
    ...
    return `
      <article class="catalog-card" data-catalog-text="${escapeHTML(`${thread.title || ''} ${thread.name || ''} ${thread.comment || ''}`.toLowerCase())}">
        ...
        <div class="catalog-date">${this.formatDate(thread.createdAt)}</div>
  ...
  archive(threads, board, stats) {
    ...
    return this.shell(
      ...
      board,
      stats,
      `<main class="archive-container"><div class="catalog-header-row"><h2>Archive</h2><span>Read-only threads</span></div>${content}</main>`
    );
  }
...
<body>
  ...
  adminPost(post, thread, board, csrf, staff, isOp = false) {
    ...
    const staffReply = !thread.archived && staffCan(staff, 'posts.capcode', board.id)
      ...
      : '';
    return `<div class="admin-post-summary"><div><strong>No.${post.id}${post.title ? ` — ${escapeHTML(post.title)}` : ''}</strong> <span class="admin-muted">by ${escapeHTML(post.name || this.anonymousName(board))}${post.capcode ? ` · ## ${escapeHTML(roleLabel(post.capcode))}` : ''} · ${this.formatDate(post.createdAt)}${post.editedAt ? ` · edited ${this.formatDate(post.editedAt)}` : ''}</span></div><p>${escapeHTML(previewText(post.comment, 240))}</p><div class="admin-post-actions">${deletion}${controls}${editing}${staffReply}${sanction}</div></div>`;
  }
  ...
  adminDashboard(data, csrf, staff) {
    ...
    const bans = activeBans.map(ban => `<li><strong>${escapeHTML(ban.kind)}</strong> · ${escapeHTML(ban.target)} · ${escapeHTML(ban.scope === 'board' ? `/${ban.boardId}/` : 'global')} — ${escapeHTML(ban.reason)}${ban.moderatorNote ? ` <span class="admin-muted">(${escapeHTML(ban.moderatorNote)})</span>` : ''} — ${ban.kind === 'warning' ? 'pending delivery' : (ban.expiresAt ? `until ${this.formatDate(ban.expiresAt)}` : 'permanent')}<form action="/admin/unban" method="POST" class="admin-action-form">${this.csrf(csrf)}<input type="hidden" name="banId" value="${escapeHTML(ban.id)}"><button type="submit">Lift</button></form></li>`).join('');
  ...
  adminRevisions(data, csrf, filters = {}, staff) {
    ...
    return this.adminShell(
      ...
      `<section class="admin-panel"><div class="admin-toolbar"><div><h1>Edit history</h1><p>Immutable staff revision records${requestedPostId ? ` for No.${requestedPostId}` : ''}.</p></div><div class="admin-toolbar-actions"><a href="/admin">Dashboard</a><a href="/admin/trash">Trash</a><form action="/admin/logout" method="POST">${this.csrf(csrf)}<button type="submit">Log out</button></form></div></div><form action="/admin/revisions" method="GET" class="admin-report-filters"><label>Post number <input type="number" name="postId" min="1" value="${requestedPostId || ''}"></label><button type="submit">Filter</button></form><section class="admin-report-list">${cards || '<p>No matching revisions.</p>'}</section></section>`
    );
  }
  ...
  adminAppeals(data, csrf, filters = {}, staff) {
    ...
    return this.adminShell(
      ...
      `<section class="admin-panel"><div class="admin-toolbar"><div><h1>Appeals</h1><p>${appeals.length} matching appeal${appeals.length === 1 ? '' : 's'}.</p></div><div class="admin-toolbar-actions"><a href="/admin">Dashboard</a><a href="/admin/reports">Reports</a>${staffCan(staff, 'staff.manage') ? '<a href="/admin/staff">Staff</a>' : ''}<form action="/admin/logout" method="POST">${this.csrf(csrf)}<button type="submit">Log out</button></form></div></div><p>Status: ${statusLinks}</p><section class="admin-report-list">${cards || '<p>No matching appeals.</p>'}</section></section>`
    );
  }
