...
const { MongoPostRepository } = require('./mongo-post-repository');
const {
  decryptSecret,
  encryptSecret,
...
function siteStats(data, uploadDir) {
  ...
  const activeContent = uploadDir
    ? directorySize(uploadDir)
    : data.media.filter(asset => asset.state === 'approved')
      .reduce((total, asset) => total + (Number(asset.bytes) || 0), 0);
  return {
    threadCount: threads.length,
    replyCount,
    postCount,
...
function commentHostnames(comment) {
  const hostnames = new Set();
  for (const match of String(comment || '').matchAll(/https?:\/\/[^\s<>"']+/gi)) {
    try {
      const hostname = new URL(match[0]).hostname.toLowerCase();
      if (hostname) hostnames.add(hostname);
    } catch {
      // Ignore malformed URLs; they carry no hostname to match.
    }
  }
  return hostnames;
}

function boardFilterMatch(filters, fields) {
  const list = Array.isArray(filters) ? filters : [];
  if (!list.length) return null;
  const haystack = `${fields.rawName || ''}\n${fields.title || ''}\n${fields.comment || ''}`.toLowerCase();
  let hostnames = null;
  for (const filter of list) {
    const value = String(filter?.value || '').toLowerCase();
    if (!value) continue;
    if (filter.kind === 'literal') {
      if (haystack.includes(value)) return filter;
      continue;
    }
    if (filter.kind === 'domain') {
      hostnames = hostnames ?? commentHostnames(fields.comment);
      for (const hostname of hostnames) {
        if (hostname === value || hostname.endsWith(`.${value}`)) return filter;
      }
    }
  }
  return null;
...
class BoardService {
  constructor(config, store, uploads) {
    ...
    this.logger = uploads.logger || console;
    this.mongoPosts = store.supportsTargetedQueries
      ? new MongoPostRepository(config, store, this)
      : null;
  }
  ...
  get usesTargetedQueries() {
    return this.store.supportsTargetedQueries === true;
  }

  async publicBoards() {
    return this.usesTargetedQueries ? this.store.publicBoards() : this.getBoards();
  }

  async publicBoard(uri) {
    return this.usesTargetedQueries ? this.store.boardByUri(uri) : this.getBoard(uri);
  }

  async publicDefaultBoard() {
    if (!this.usesTargetedQueries) return this.getDefaultBoard();
    const boards = await this.store.publicBoards();
    const configuredUri = this.config.board?.uri || defaultBoardUri(this.config);
    return boards.find(board => board.uri === configuredUri) || boards[0] || null;
  }

  async publicCustomization() {
    return this.usesTargetedQueries ? this.store.customization() : this.getCustomization();
  }

  async publicCustomPage(slug) {
    const customization = await this.publicCustomization();
    return (customization.pages || []).find(page => page.slug === String(slug || '').toLowerCase()) || null;
  }

  async resolveStaffSessionFresh(session) {
    if (!session || !this.usesTargetedQueries) return this.resolveStaffSession(session);
    ...
    const account = await this.store.staffById(session.accountId);
    if (!account || account.enabled === false
      || Number(session.sessionVersion) !== Number(account.sessionVersion)) return null;
    return this.publicStaffAccount(account);
  }

  async publicThread(threadId, boardId = '') {
    if (this.usesTargetedQueries) return this.store.readThread(threadId, boardId);
    const data = this.getData();
    const thread = this.getThread(threadId, data);
    if (!thread || (boardId && thread.boardId !== String(boardId))) return null;
    const board = this.getBoardById(thread.boardId, data);
    if (!board || !board.enabled) return null;
    return { data, thread, board, stats: this.getStats(data, board.uri) };
  }

  async publicBoardPage(board, page = 1) {
    if (this.usesTargetedQueries) {
      return this.store.readBoardPage(board.id, page, this.config.limits.threadsPerPage);
    }
    return this.getPage(page, board.uri);
  }

  async publicCatalog(board, page = 1) {
    if (this.usesTargetedQueries) {
      return this.store.readCatalog(board.id, {
        page,
        limit: this.config.limits.catalogThreadsPerPage
      });
    }
    const data = this.getData();
    const allThreads = this.getSortedThreads(data, board.id);
    const limit = this.config.limits.catalogThreadsPerPage;
    const currentPage = Math.max(1, Number.parseInt(page, 10) || 1);
    return {
      data,
      threads: allThreads.slice((currentPage - 1) * limit, currentPage * limit),
      page: currentPage,
      totalPages: Math.max(1, Math.ceil(allThreads.length / limit)),
      stats: this.getStats(data, board.uri)
    };
  }

  async publicArchive(board, page = 1) {
    if (this.usesTargetedQueries) {
      return this.store.readArchive(board.id, {
        page,
        limit: this.config.limits.catalogThreadsPerPage
      });
    }
    const data = this.getData();
    const allThreads = this.getArchivedThreads(data, board.id);
    const limit = this.config.limits.catalogThreadsPerPage;
    const currentPage = Math.max(1, Number.parseInt(page, 10) || 1);
    return {
      data,
  ...
  async publicOverboard(options = {}) {
    const sfw = options.sfw === true;
    const rawTag = String(options.tag || '').trim().toLowerCase();
    const tag = BOARD_TAG_PATTERN.test(rawTag) ? rawTag : '';
    const attachMedia = (entries, data) => entries.map(entry => ({
      ...entry,
      media: this.overboardMedia(entry.thread, data)
    }));
    if (this.usesTargetedQueries) {
      const view = await this.store.readOverboard({
        page: options.page,
        limit: OVERBOARD_PAGE_SIZE,
        sfw,
        tag
      });
      if (!view) return null;
      return { ...view, entries: attachMedia(view.entries, view.data), sfw, tag };
    }
    const data = this.getData();
    const all = this.overboardEntries(data, { sfw, tag });
    ...
    const totalPages = Math.max(1, Math.ceil(all.length / OVERBOARD_PAGE_SIZE));
    if (currentPage > totalPages) return null;
    const start = (currentPage - 1) * OVERBOARD_PAGE_SIZE;
    return {
      data,
      entries: attachMedia(all.slice(start, start + OVERBOARD_PAGE_SIZE), data),
      page: currentPage,
      totalPages,
      sfw,
      tag
  ...
  latestImagesFromEntries(entries, limit = 24, inspectLocal = false) {
    const images = [];
    const seen = new Set();
    for (const entry of entries) {
      for (const attachment of postAttachments(entry.post)) {
        if (inspectLocal) {
          if (!this.uploads.canReference(attachment.image)) continue;
        }
        const key = attachment.contentSha256 || attachment.sha256 || attachment.image;
        if (!attachment.image || seen.has(key)) continue;
        seen.add(key);
        images.push({
          ...entry,
          post: {
  ...
  async publicHome() {
    if (this.usesTargetedQueries) {
      const view = await this.store.readHome();
      return {
        ...view,
        latestImages: this.latestImagesFromEntries(view.latestImageCandidates, 24)
      };
    }
    const data = this.getData();
    return {
      ...
      stats: this.getSiteStats(data),
      latestPosts: this.latestPosts(30, data),
      latestImages: this.latestImages(24, data)
    };
  }

  async publicSearch(query) {
    return this.usesTargetedQueries ? this.store.searchPosts(query, 100) : this.search(query);
  }

  async approvedMediaRecord(filename) {
    return this.usesTargetedQueries ? this.store.approvedMedia(filename) : this.approvedMedia(filename);
  }
  ...
  moderationDataFor(staff, data = this.getData()) {
    ...
    const scopedThreads = data.threads.filter(thread => boardIds.has(thread.boardId));
    const scopedTrash = data.trash.filter(entry => boardIds.has(entry.boardId));
    const mediaIds = new Set();
    for (const thread of scopedThreads) {
      for (const post of [thread, ...thread.replies]) {
        for (const attachment of postAttachments(post)) {
          if (attachment.assetId) mediaIds.add(attachment.assetId);
        }
      }
    }
    for (const entry of scopedTrash) {
      const posts = entry.kind === 'thread' ? [entry.post, ...entry.post.replies] : [entry.post];
    ...
    return {
      ...
      media: data.media.filter(asset => mediaIds.has(asset.id)),
      ...
      mediaHashBans: data.mediaHashBans.filter(entry => entry.scope === 'board' && boardIds.has(entry.boardId)),
      mediaDecisions: data.mediaDecisions.filter(entry => entry.boardId && boardIds.has(entry.boardId)),
  ...
  async moderationQueueFor(staff, queue, filters = {}) {
    if (this.usesTargetedQueries) {
      return this.store.readModerationQueue(queue, {
        ...filters,
        limit: this.config.limits.adminPageSize
      }, staff);
    }
    return { data: this.moderationDataFor(staff), pageInfo: null };
  }

  mfaLoginUpdate(account, code) {
    if (account.mfaEnabled !== true) return {};
    let key;
    let secret;
    try {
      key = parseEncryptionKey(this.config.staffMfa.encryptionKey);
      secret = decryptSecret(account.mfaSecret, key);
    } catch {
      return null;
    }
    const counter = verifyTotp(secret, code, { lastCounter: account.mfaLastCounter });
    if (counter !== null) return { totpCounter: counter };
    const recoveryHash = hashRecoveryCode(code, key);
    if (recoveryHash && account.mfaRecoveryHashes?.includes(recoveryHash)) return { recoveryHash };
    return null;
  }

  async authenticateStaff(username, password, mfaCode = '') {
    const normalizedUsername = normalizeUsername(username);
    if (this.usesTargetedQueries) {
      const candidate = await this.store.staffByUsername(normalizedUsername);
      ...
      const mfaUpdate = this.mfaLoginUpdate(candidate, mfaCode);
      if (!mfaUpdate) return null;
      return this.store.recordStaffLogin(candidate, mfaUpdate);
    }
    ...
    return (await this.store.update(data => {
      ...
      if (account.mfaEnabled === true) {
        if (Number.isSafeInteger(mfaUpdate.totpCounter)) {
          if (Number(account.mfaLastCounter) >= mfaUpdate.totpCounter) return null;
          account.mfaLastCounter = mfaUpdate.totpCounter;
        } else if (mfaUpdate.recoveryHash) {
          const recoveryIndex = (account.mfaRecoveryHashes || []).indexOf(mfaUpdate.recoveryHash);
          if (recoveryIndex < 0) return null;
          account.mfaRecoveryHashes.splice(recoveryIndex, 1);
        } else {
          return null;
        }
      }
  ...
  async beginStaffMfa(currentPassword, actor) {
    if (!this.config.staffMfa.enabled) throw httpError(404, 'Staff MFA enrollment is not enabled.');
    if (!actor || actor.legacy) throw httpError(400, 'MFA enrollment requires a named staff account.');
    const key = parseEncryptionKey(this.config.staffMfa.encryptionKey);
    const secret = generateTotpSecret();
    const recoveryCodes = generateRecoveryCodes();
    const encryptedSecret = encryptSecret(secret, key);
    const recoveryHashes = recoveryCodes.map(code => hashRecoveryCode(code, key));
    const account = (await this.store.update(data => {
      ...
      const stored = data.staff.find(item => item.id === currentActor.id);
      const passwordValid = verifyPassword(currentPassword, stored?.passwordHash || DUMMY_STAFF_PASSWORD_HASH);
      if (!stored || !passwordValid) throw httpError(403, 'Current password is incorrect.');
      if (stored.mfaEnabled) throw httpError(409, 'MFA is already enabled for this account.');
      stored.mfaPendingSecret = encryptedSecret;
      stored.mfaPendingRecoveryHashes = recoveryHashes;
      stored.mfaPendingAt = Date.now();
      stored.updatedAt = Date.now();
      this.logModeration(data, 'staff-mfa-begin', `Started MFA enrollment for ${stored.username}`, {
        actor: currentActor
      });
      return this.publicStaffAccount(stored);
    })).result;
    return {
      account,
      secret,
      recoveryCodes,
      uri: totpUri(secret, account.username, this.config.staffMfa.issuer)
    };
  }

  async confirmStaffMfa(code, actor) {
    if (!this.config.staffMfa.enabled || !actor || actor.legacy) {
      throw httpError(400, 'MFA enrollment is unavailable for this account.');
    }
    const key = parseEncryptionKey(this.config.staffMfa.encryptionKey);
    return (await this.store.update(data => {
      const currentActor = this.currentStaffActor(actor, data);
      const account = data.staff.find(item => item.id === currentActor?.id);
      if (!account?.mfaPendingSecret || !(account.mfaPendingRecoveryHashes || []).length) {
        throw httpError(409, 'Start MFA enrollment before confirming it.');
      }
      let secret;
      try {
        secret = decryptSecret(account.mfaPendingSecret, key);
      } catch {
        throw httpError(503, 'Stored MFA enrollment data could not be decrypted.');
      }
      if (verifyTotp(secret, code, { lastCounter: -1 }) === null) {
        throw httpError(403, 'The authentication code is invalid.');
      }
      account.mfaEnabled = true;
      account.mfaSecret = account.mfaPendingSecret;
      account.mfaRecoveryHashes = account.mfaPendingRecoveryHashes;
      account.mfaLastCounter = -1;
      account.mfaEnabledAt = Date.now();
      delete account.mfaPendingSecret;
      delete account.mfaPendingRecoveryHashes;
      delete account.mfaPendingAt;
      account.sessionVersion += 1;
      account.updatedAt = Date.now();
      this.logModeration(data, 'staff-mfa-enable', `Enabled MFA for ${account.username}`, { actor: currentActor });
      return this.publicStaffAccount(account);
    })).result;
  }

  async disableStaffMfa(currentPassword, code, actor) {
    if (!actor || actor.legacy) throw httpError(400, 'MFA is unavailable for this account.');
    const key = parseEncryptionKey(this.config.staffMfa.encryptionKey);
    return (await this.store.update(data => {
      const currentActor = this.currentStaffActor(actor, data);
      const account = data.staff.find(item => item.id === currentActor?.id);
      const passwordValid = verifyPassword(currentPassword, account?.passwordHash || DUMMY_STAFF_PASSWORD_HASH);
      if (!account || !passwordValid) throw httpError(403, 'Current credentials are incorrect.');
      if (!account.mfaEnabled) throw httpError(409, 'MFA is not enabled for this account.');
      let secret;
      try {
        secret = decryptSecret(account.mfaSecret, key);
      } catch {
        throw httpError(503, 'Stored MFA data could not be decrypted.');
      }
      const counter = verifyTotp(secret, code, { lastCounter: account.mfaLastCounter });
      const recoveryHash = hashRecoveryCode(code, key);
      if (counter === null && (!recoveryHash || !account.mfaRecoveryHashes?.includes(recoveryHash))) {
        throw httpError(403, 'Current credentials are incorrect.');
      }
      account.mfaEnabled = false;
      delete account.mfaSecret;
      delete account.mfaRecoveryHashes;
      account.mfaLastCounter = -1;
      account.mfaEnabledAt = 0;
      delete account.mfaPendingSecret;
      delete account.mfaPendingRecoveryHashes;
      delete account.mfaPendingAt;
      account.sessionVersion += 1;
      account.updatedAt = Date.now();
      this.logModeration(data, 'staff-mfa-disable', `Disabled MFA for ${account.username}`, { actor: currentActor });
      return this.publicStaffAccount(account);
    })).result;
  }
  ...
  getStats(data = this.getData(), boardUri = null) {
    ...
    return siteStats(data, this.config.mediaStorage.backend === 'local' ? this.config.uploadDir : null);
  }
  ...
  findMediaHashBan(hashes, boardId, data = this.getData()) {
    const candidates = new Set((Array.isArray(hashes) ? hashes : [hashes])
      .map(value => String(value || '').toLowerCase())
      .filter(value => /^[a-f0-9]{64}$/.test(value)));
    return data.mediaHashBans.find(entry => entry.active !== false
      && candidates.has(entry.sha256)
      && (entry.scope !== 'board' || entry.boardId === String(boardId || ''))) || null;
  }

  async findMediaHashBanRecord(hashes, boardId) {
    return this.usesTargetedQueries
      ? this.store.findMediaHashBan(hashes, boardId)
      : this.findMediaHashBan(hashes, boardId);
  }

  async recordMediaDecision(fields) {
    const decision = {
      id: crypto.randomUUID(),
      ...
      decision: String(fields.decision || 'rejected'),
      reasonCode: String(fields.reasonCode || '').replace(/[^a-z0-9:_-]/gi, '').slice(0, 80),
      reason: cleanText(fields.reason).slice(0, 300),
      provider: String(fields.provider || '').replace(/[^a-z0-9._-]/gi, '').slice(0, 80),
      providerReference: cleanText(fields.providerReference).slice(0, 200),
      actorId: String(fields.actorId || ''),
      actorName: String(fields.actorName || ''),
      createdAt: Date.now()
    };
    if (this.usesTargetedQueries) return this.store.insertMediaDecision(decision);
    return (await this.store.update(data => {
      data.mediaDecisions.push(decision);
      data.mediaDecisions = data.mediaDecisions.slice(-5000);
      return decision;
    })).result;
  }

  async recordMediaProviderResult(fields) {
    const result = {
      id: crypto.randomUUID(),
      sha256: String(fields.sha256 || '').toLowerCase(),
      provider: String(fields.provider || '').replace(/[^a-z0-9._-]/gi, '').slice(0, 80),
      available: fields.available === true,
      matched: fields.matched === true,
      reasonCode: String(fields.reasonCode || '').replace(/[^a-z0-9:_-]/gi, '').slice(0, 80),
      providerReference: cleanText(fields.providerReference).slice(0, 200),
      checkedAt: Date.now()
    };
    if (this.usesTargetedQueries) return this.store.insertMediaProviderResult(result);
    return (await this.store.update(data => {
      data.mediaProviderResults.push(result);
      data.mediaProviderResults = data.mediaProviderResults.slice(-5000);
      return result;
    })).result;
  }

  async recordAutomatedMediaRejection(fields) {
    if (this.usesTargetedQueries) return this.store.recordAutomatedMediaRejection(fields);
    return (await this.store.update(data => {
      const now = Date.now();
      const sha256 = String(fields.sha256 || '').toLowerCase();
      let hashBan = data.mediaHashBans.find(entry => entry.sha256 === sha256
        && entry.scope === 'global' && !entry.boardId);
      if (!hashBan) {
        hashBan = {
          id: crypto.randomUUID(),
          sha256,
        ...
        data.mediaHashBans.push(hashBan);
      } else {
        hashBan.active = true;
        hashBan.updatedAt = now;
      }
      const decision = {
        id: crypto.randomUUID(),
        sha256,
      ...
      data.mediaDecisions.push(decision);
      data.mediaDecisions = data.mediaDecisions.slice(-5000);
      this.logModeration(data, 'media-provider-reject', `Rejected media hash ${sha256}`, {
        actor: { id: 'system', username: 'media-safety' },
        boardId: decision.boardId
      });
      return { hashBan, decision };
    })).result;
  }
  ...
  postingRestriction(data, posterKey, boardId, upload) {
    ...
    const uploadHashes = new Set(uploadCandidates(upload).flatMap(candidate => [
      candidate.sha256,
      candidate.contentSha256,
      candidate._asset?.sourceSha256,
      candidate._asset?.contentSha256
    ]).filter(Boolean));
    const mediaHashBan = data.mediaHashBans.find(entry => entry.active !== false
      && uploadHashes.has(entry.sha256)
      && (entry.scope !== 'board' || entry.boardId === boardId));
    if (mediaHashBan) {
      return {
        ...mediaHashBan,
        kind: 'ban',
  ...
  banError(sanction) {
    ...
    if (sanction.appealId) error.appealUrl = `/appeals/${encodeURIComponent(sanction.appealId)}`;
    return error;
  }
  ...
  assertBoardFilters(board, fields) {
    const match = boardFilterMatch(board?.filters, fields);
    if (match) throw httpError(400, String(match.note || '').trim() || 'Blocked by a board filter.');
  }

  registerUpload(data, upload) {
    if (!upload?._asset) return {};
    if (upload._asset.state !== 'approved' || !upload._asset.path) {
    ...
    if (asset) {
      if (asset.state !== 'approved') throw httpError(409, 'Matching media is not approved for publication.');
    ...
    return {
      ...
      metadataStripped: Boolean(asset.metadataStripped)
    };
  }
  ...
  recordApprovedUploads(data, uploads, boardId, context = {}) {
    const actor = context.actor;
    const now = Date.now();
    for (const upload of uploadCandidates(uploads)) {
      const sha256 = String(upload?.sha256 || upload?._asset?.sourceSha256 || '').toLowerCase();
      if (!/^[a-f0-9]{64}$/.test(sha256)) continue;
      data.mediaDecisions.push({
        id: crypto.randomUUID(),
        sha256,
        contentSha256: String(upload?.contentSha256 || upload?._asset?.contentSha256 || '').toLowerCase(),
        ...
        actorName: String(actor?.username || actor?.displayName || 'media-pipeline'),
        createdAt: now
      });
    }
    data.mediaDecisions = data.mediaDecisions.slice(-5000);
  ...
  async cleanupReleasedMedia(releasedAssets, legacyPosts) {
    await Promise.all([
      ...releasedAssets.map(asset => this.uploads.removeAsset(asset)),
      ...legacyPosts.map(post => this.uploads.removePost(post))
    ]);
  }

  mediaIdsForPosts(posts) {
    const ids = new Set();
    for (const post of posts) {
      for (const attachment of postAttachments(post)) {
  ...
  planAssetHold(asset, heldAssets, now = Date.now(), reason = 'staff-trash') {
    if (!asset || asset.state !== 'approved') return false;
    const hold = this.uploads.planHold(asset);
    asset.state = 'moderator_hold';
    asset.path = '';
    asset.thumbnail = '';
    asset.heldAt = Number(now);
    asset.holdReason = reason === 'hash-ban' ? 'hash-ban' : 'staff-trash';
    if (!hold) return true;
    asset.hold = hold;
    asset.holdPending = true;
    heldAssets.push(structuredClone(asset));
    return true;
  }

  planModerationHolds(data, removedPosts, heldAssets, now = Date.now()) {
    const candidates = this.mediaIdsForPosts(removedPosts);
    if (!candidates.size) return;
    const active = this.mediaIdsForPosts(allPosts(data).map(entry => entry.post));
    for (const asset of data.media) {
      if (!candidates.has(asset.id) || active.has(asset.id) || asset.state !== 'approved') continue;
      this.planAssetHold(asset, heldAssets, now);
      if (asset.sha256) {
        data.mediaDecisions.push({
          id: crypto.randomUUID(),
          sha256: asset.sha256,
          ...
          boardId: '',
          decision: 'moderator_hold',
          reasonCode: 'staff-trash',
          reason: 'Media was moved to staff trash and removed from public delivery.',
          provider: '',
          providerReference: '',
          actorId: 'system',
          actorName: 'moderation-pipeline',
  ...
  async completeMediaHolds(heldAssets) {
    if (!heldAssets.length) return;
    const results = await Promise.allSettled(heldAssets.map(asset => this.uploads.holdAsset(asset)));
    const completed = heldAssets.filter((asset, index) => results[index].status === 'fulfilled');
    if (completed.length) {
      const completedIds = new Set(completed.map(asset => asset.id));
      await this.store.update(data => {
        for (const asset of data.media) {
          if (completedIds.has(asset.id) && asset.state === 'moderator_hold') asset.holdPending = false;
        }
      });
    }
    const failed = results.filter(result => result.status === 'rejected');
    if (failed.length) {
      this.logger.error('One or more public media objects could not be moved to private moderation storage.', {
        category: 'media-hold-failed',
        failedCount: failed.length
      });
      throw httpError(503, 'The content is no longer available through ChikoChan, but private storage relocation is still pending.');
    }
  }

  validateTrashRestore(data, entry) {
    ...
    if (entry.kind === 'attachment') {
      ...
      } else if (postAttachments(target.post).length) {
      ...
      if (!this.getThread(entry.threadId, data)) {
        throw httpError(409, 'Restore the parent thread before restoring this reply.');
      }
    }
  }

  applyTrashRestore(data, entry) {
    this.validateTrashRestore(data, entry);
    if (entry.kind === 'attachment') {
      const target = findPost(data, entry.postId);
      if (entry.attachmentId) {
        const current = postAttachments(target.post);
        current.splice(
          ...
          0,
          ...structuredClone(postAttachments(entry.post))
        );
  ...
  async createThread(input, image, context) {
    if (this.mongoPosts?.available) return this.mongoPosts.createThread(input, image, context);
    const uploads = uploadCandidates(image);
    const transaction = await this.store.update(data => {
      ...
      this.assertBoardFilters(board, fields);
      ...
      this.recordApprovedUploads(data, uploads, board.id, context);
      if (context.actor) {
    ...
    for (const upload of uploads) {
      if (upload._deduplicated) await this.uploads.removeCandidate(upload);
    }
    return transaction.result;
  }

  async createReply(threadId, input, image, context) {
    if (this.mongoPosts?.available) return this.mongoPosts.createReply(threadId, input, image, context);
    const uploads = uploadCandidates(image);
    ...
    const transaction = await this.store.update(data => {
      ...
      if (thread.replies.length >= replyLimit) {
        ...
        if (removed) {
          ...
          this.closeRelatedReports(data, new Set([removed.id]));
        }
      }
      ...
      this.recordApprovedUploads(data, uploads, thread.boardId, context);
    ...
    await this.cleanupReleasedMedia(releasedAssets, legacyPosts);
    return transaction.result;
  }
  ...
  async deleteByPassword(rawIds, password, fileOnly = false) {
    ...
    if (ids.length > 20) throw httpError(400, 'At most 20 posts can be selected for one deletion request.');
    if (this.mongoPosts?.available) return this.mongoPosts.deleteByPassword(ids, password, fileOnly);
    const releasedAssets = [];
    const legacyPosts = [];

    const transaction = await this.store.update(data => {
      const targets = ids.map(id => findPost(data, id));
      const authorized = targets.map(target => verifyPassword(
        password,
        target?.post?.passwordHash || DUMMY_STAFF_PASSWORD_HASH
      ));
      if (targets.some(target => !target) || authorized.some(valid => !valid)) {
  ...
  async reportPost(postId, reason, options = {}) {
    ...
    if (this.usesTargetedQueries) {
      const metadata = await this.store.siteMetadata();
      if (!metadata?.meta?.siteSecret) throw new Error('Site metadata is unavailable.');
      return this.store.createReport({
        postId: Number.parseInt(postId, 10),
        reason: cleanReason,
        category,
        reporterKey: this.reporterFingerprint(options.clientKey, metadata)
      });
    }
    return (await this.store.update(data => {
  ...
  async adminDelete(postId, context = {}) {
    ...
    const transaction = await this.store.update(data => {
      ...
      if (fileOnly) {
        ...
        this.planModerationHolds(data, [snapshot], heldAssets, now);
        return {
      ...
      this.planModerationHolds(
        data,
        target.isThread ? [snapshot, ...snapshot.replies] : [snapshot],
        heldAssets,
        now
      );
      return { deleted: deletedIds, fileOnly: false, trashId: trash.id };
    });
    await this.completeMediaHolds(heldAssets);
    return transaction.result;
  }
  ...
  async restoreTrash(trashId, context = {}) {
    ...
    await this.store.update(data => {
      ...
      if (entry.purgeAt <= claimedAt) throw httpError(410, 'This trash entry has expired and cannot be restored.');
      if (entry.restoringToken && Number(entry.restoringAt) > claimedAt - 10 * 60 * 1000) {
        throw httpError(409, 'This trash entry is already being restored.');
      }
      this.validateTrashRestore(data, entry);
      entry.restoringToken = restoreToken;
      entry.restoringAt = claimedAt;
      const posts = entry.kind === 'thread' ? [entry.post, ...entry.post.replies] : [entry.post];
      const assetIds = this.mediaIdsForPosts(posts);
      const blockedAsset = data.media.find(asset => assetIds.has(asset.id)
        && asset.state === 'moderator_hold' && asset.holdReason === 'hash-ban');
      if (blockedAsset) throw httpError(409, 'Media covered by an active hash ban cannot be restored.');
      heldAssets = data.media
        .filter(asset => assetIds.has(asset.id) && asset.state === 'moderator_hold'
          && asset.holdReason === 'staff-trash' && asset.hold)
        .map(asset => structuredClone(asset));
    });

    try {
      await Promise.all(heldAssets.map(asset => this.uploads.restoreHeldAsset(asset)));
      return (await this.store.update(data => {
        const index = data.trash.findIndex(entry => entry.id === String(trashId || ''));
        if (index < 0 || data.trash[index].restoringToken !== restoreToken) {
          throw httpError(409, 'The trash restore claim is no longer valid.');
        }
        const entry = data.trash[index];
        this.assertStaffPermission(context, 'posts.delete', entry.boardId, data);
        this.applyTrashRestore(data, entry);
        const restoredAssetIds = new Set(heldAssets.map(asset => asset.id));
        for (const asset of data.media) {
          if (!restoredAssetIds.has(asset.id) || asset.state !== 'moderator_hold' || !asset.hold) continue;
          asset.path = asset.hold.originalPath;
          asset.thumbnail = asset.hold.originalThumbnail;
          asset.state = 'approved';
          asset.approvedAt = Number(asset.approvedAt) || Date.now();
          delete asset.hold;
          delete asset.holdPending;
          delete asset.heldAt;
          delete asset.holdReason;
        }
      ...
      const rollback = await Promise.allSettled(heldAssets.map(asset => this.uploads.holdAsset(asset)));
      if (rollback.some(result => result.status === 'rejected')) {
        this.logger.error('Media restore rollback requires storage reconciliation.', {
          category: 'media-restore-rollback-failed',
          trashId: String(trashId || '')
        });
      }
      await this.store.update(data => {
        const entry = data.trash.find(item => item.id === String(trashId || ''));
        if (entry?.restoringToken === restoreToken) {
          delete entry.restoringToken;
  ...
  async purgeExpiredTrash(context = {}, now = Date.now()) {
    ...
    const transaction = await this.store.update(data => {
      ...
      const expired = data.trash.filter(entry => Number(entry.purgeAt) <= Number(now)
        && (!entry.restoringToken || Number(entry.restoringAt) <= Number(now) - 10 * 60 * 1000)
  ...
  async performMaintenance(now = Date.now()) {
    ...
    const transaction = await this.store.update(data => {
      ...
      for (const asset of data.media) {
        if ((Number(asset.refCount) || 0) > 0 && asset.state === 'moderator_hold'
          && asset.holdPending && asset.hold) pendingHolds.push(structuredClone(asset));
    ...
    await Promise.all(orphanAssets.map(asset => this.uploads.removeAsset(asset)));
    await this.completeMediaHolds(pendingHolds);
    return transaction.result;
  }
  ...
  async resolveReport(reportId, resolution = 'dismissed', note = '', context = {}) {
    ...
    if (this.usesTargetedQueries && this.store.supportsTransactions) {
      this.assertStaffPermission(context, 'reports.manage');
      return this.store.updateReportStatus(reportId, {
        action: 'resolved',
  ...
  async reopenReport(reportId, context = {}) {
    if (this.usesTargetedQueries && this.store.supportsTransactions) {
      this.assertStaffPermission(context, 'reports.manage');
      return this.store.updateReportStatus(reportId, { action: 'reopened' }, context);
    }
    return (await this.store.update(data => {
      const report = data.reports.find(item => item.id === String(reportId || ''));
      if (!report) throw httpError(404, 'Report not found.');
  ...
  async sanctionPost(postId, options = {}, context = {}) {
    ...
    const transaction = await this.store.update(data => {
      ...
      if (sanctionTarget === 'file') {
        ...
        const existingHashBan = data.mediaHashBans.find(entry => entry.sha256 === fileHash
          && entry.scope === mediaHashBan.scope && entry.boardId === mediaHashBan.boardId);
        if (existingHashBan) Object.assign(existingHashBan, mediaHashBan, { id: existingHashBan.id });
        else data.mediaHashBans.push(mediaHashBan);
        data.mediaDecisions.push({
          id: crypto.randomUUID(),
          sha256: fileHash,
          contentSha256: postAttachments(target.post)
            .find(attachment => attachment.sha256 === fileHash)?.contentSha256 || '',
          boardId: target.thread.boardId,
          decision: 'rejected',
          reasonCode: 'staff-hash-ban',
          reason: cleanReason,
          provider: '',
          providerReference: '',
          actorId: String(actor?.id || ''),
        ...
        if (scope === 'global') {
          for (const asset of data.media) {
            if (![asset.sha256, asset.sourceSha256, asset.contentSha256].includes(fileHash)) continue;
            if (asset.state === 'moderator_hold') asset.holdReason = 'hash-ban';
            else this.planAssetHold(asset, heldAssets, now, 'hash-ban');
          }
        }
      }
  ...
  async createMediaHashBan(fields = {}, context = {}) {
    const sha256 = String(fields.sha256 || '').trim().toLowerCase();
    if (!/^[a-f0-9]{64}$/.test(sha256)) throw httpError(400, 'A valid SHA-256 hash is required.');
    const reason = cleanText(fields.reason, 'Prohibited media').slice(0, 300);
    const moderatorNote = cleanText(fields.moderatorNote).slice(0, 500);
    let heldAssets = [];
    const transaction = await this.store.update(data => {
      heldAssets = [];
      const actor = this.currentStaffActor(context.actor, data);
      if (!actor) throw httpError(403, 'Your staff session is no longer valid.');
      const requestedBoard = data.boards.find(board => board.id === String(fields.boardId || ''));
      const scope = fields.scope === 'board' || actor.scope === 'boards' ? 'board' : 'global';
      const boardId = scope === 'board' ? String(requestedBoard?.id || '') : '';
      if (scope === 'board' && !boardId) throw httpError(400, 'Choose a valid board for this hash ban.');
      this.assertStaffPermission({ actor }, 'bans.manage', boardId, data);
      if (scope === 'global' && actor.scope !== 'global') {
        throw httpError(403, 'Board-scoped staff cannot create global media bans.');
      }
      const now = Date.now();
      let entry = data.mediaHashBans.find(item => item.sha256 === sha256
        && item.scope === scope && item.boardId === boardId);
      if (entry) {
        Object.assign(entry, {
          active: true,
          reason,
      ...
      if (scope === 'global') {
        for (const asset of data.media) {
          if (![asset.sha256, asset.sourceSha256, asset.contentSha256].includes(sha256)) continue;
          if (asset.state === 'moderator_hold') asset.holdReason = 'hash-ban';
          else this.planAssetHold(asset, heldAssets, now, 'hash-ban');
        }
      }
      this.logModeration(data, 'media-hash-ban', `Created ${scope} media hash ban ${sha256}`, {
        actor,
        boardId
      });
  ...
  async liftMediaHashBan(hashBanId, context = {}) {
    return (await this.store.update(data => {
      const entry = data.mediaHashBans.find(item => item.id === String(hashBanId || ''));
      if (!entry) throw httpError(404, 'Media hash ban not found.');
      this.assertStaffPermission(context, 'bans.manage', entry.boardId, data);
      const actor = this.currentStaffActor(context.actor, data) || context.actor;
      if (entry.scope === 'global' && actor.scope !== 'global') {
        throw httpError(403, 'Board-scoped staff cannot lift global media bans.');
      }
      entry.active = false;
      entry.updatedAt = Date.now();
      this.logModeration(data, 'media-hash-unban', `Lifted ${entry.scope} media hash ban ${entry.sha256}`, {
        ...context,
        boardId: entry.boardId
      });
      return structuredClone(entry);
  ...
  async unban(banId, context = {}) {
    return (await this.store.update(data => {
      ...
      for (const mediaHashBan of data.mediaHashBans) {
        if (mediaHashBan.sourceSanctionId !== ban.id) continue;
        mediaHashBan.active = false;
        mediaHashBan.updatedAt = ban.updatedAt;
      }
  ...
  async updateBoard(uri, fields, context = {}) {
    ...
    return (await this.store.update(data => {
      ...
      if (fields.tags !== undefined && fields.tags !== null) {
        board.tags = this.cleanBoardTags(fields.tags);
      }
      if (fields.sfw !== undefined && fields.sfw !== null) {
        board.sfw = formBoolean(fields.sfw, true);
      }
      if (fields.settings) {
  ...
  cleanBoardTags(value) {
    const raw = (Array.isArray(value) ? value : [value])
      .flatMap(entry => String(entry || '').split(/[\s,]+/))
      .map(entry => entry.trim().toLowerCase())
      .filter(Boolean);
    if (raw.some(tag => !BOARD_TAG_PATTERN.test(tag))) {
      throw httpError(400, 'Board tags use 1–24 lowercase letters, numbers, and dashes.');
    }
    const tags = [...new Set(raw)];
    if (tags.length > MAX_BOARD_TAGS) throw httpError(400, `Boards are limited to ${MAX_BOARD_TAGS} tags.`);
    return tags;
  }

  cleanBoardFilter(fields) {
    const kind = String(fields.kind || '');
    if (!BOARD_FILTER_KINDS.has(kind)) throw httpError(400, 'Board filter kind must be literal or domain.');
    const text = cleanText(fields.value);
    if (!text || text.length > MAX_BOARD_FILTER_TEXT) {
      throw httpError(400, `Board filter values must be 1–${MAX_BOARD_FILTER_TEXT} characters.`);
    }
    const value = kind === 'domain' ? text.toLowerCase() : text;
    if (kind === 'domain' && (/\s/.test(value) || !/^[a-z0-9.-]+$/.test(value))) {
      throw httpError(400, 'Domain filters use lowercase letters, numbers, dots, and dashes.');
    }
    const note = cleanText(fields.note || '');
    if (note.length > MAX_BOARD_FILTER_TEXT) {
      throw httpError(400, `Board filter notes are limited to ${MAX_BOARD_FILTER_TEXT} characters.`);
    }
    return { kind, value, note };
  }

  async addBoardFilter(uri, fields, context = {}) {
    this.assertStaffPermission(context, 'boards.manage');
    const clean = this.cleanBoardFilter(fields);
    return (await this.store.update(data => {
      const board = this.getBoard(uri, data);
      if (!board) throw httpError(404, 'Board not found.');
      this.assertStaffPermission(context, 'boards.manage', board.id, data);
      board.filters = Array.isArray(board.filters) ? board.filters : [];
      if (board.filters.length >= MAX_BOARD_FILTERS) {
        throw httpError(409, `Boards are limited to ${MAX_BOARD_FILTERS} filters.`);
      }
      const key = `${clean.kind}:${clean.value.toLowerCase()}`;
      if (board.filters.some(filter => `${filter.kind}:${String(filter.value).toLowerCase()}` === key)) {
        throw httpError(409, 'That board filter already exists.');
      }
      const filter = { id: crypto.randomUUID(), ...clean };
      board.filters.push(filter);
      this.logModeration(data, 'board-filter-add', `Added ${filter.kind} filter to /${board.uri}/`, {
        ...context,
        boardId: board.id
      });
      return filter;
  ...
  async deleteBoardFilter(uri, filterId, context = {}) {
    this.assertStaffPermission(context, 'boards.manage');
    return (await this.store.update(data => {
      const board = this.getBoard(uri, data);
      if (!board) throw httpError(404, 'Board not found.');
      this.assertStaffPermission(context, 'boards.manage', board.id, data);
      board.filters = Array.isArray(board.filters) ? board.filters : [];
      const index = board.filters.findIndex(filter => filter.id === String(filterId || ''));
      if (index < 0) throw httpError(404, 'Board filter not found.');
      const [filter] = board.filters.splice(index, 1);
      this.logModeration(data, 'board-filter-delete', `Deleted ${filter.kind} filter from /${board.uri}/`, {
        ...context,
        boardId: board.id
      });
      return filter;
  ...
  overboardEntries(data, { sfw = false, tag = '' } = {}) {
    const boardMap = new Map(data.boards.map(board => [board.id, board]));
    return data.threads
      .map(thread => ({ post: thread, thread, threadId: thread.id, board: boardMap.get(thread.boardId) }))
      .filter(entry => entry.board && entry.board.enabled && !entry.thread.archived
        && (!sfw || entry.board.sfw !== false)
        && (!tag || (Array.isArray(entry.board.tags) && entry.board.tags.includes(tag))))
      .sort((left, right) => Number(right.thread.createdAt) - Number(left.thread.createdAt));
  }

  overboardMedia(thread, data = this.getData()) {
    for (const attachment of postAttachments(thread)) {
      const approved = data.media.find(asset => asset.state === 'approved'
        && Number(asset.refCount) > 0
        && ((attachment.assetId && asset.id === attachment.assetId)
          || asset.path === attachment.image
          || asset.thumbnail === attachment.thumbnail));
      if (approved && attachment.image && this.uploads.canReference(attachment.image)) return attachment;
    }
    return null;
  }
  ...
  approvedMedia(filename, data = this.getData()) {
    const expected = `src/${String(filename || '')}`;
    const asset = data.media.find(item => item.state === 'approved'
      && Number(item.refCount) > 0
      && (item.path === expected || item.thumbnail === expected));
    return asset ? structuredClone(asset) : null;
  }
}

module.exports = { BoardService, boardStats, boardFilterMatch, findPost, sortedThreads, siteStats };
