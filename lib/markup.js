...
function buildPostIndex(data) {
  ...
  for (const thread of data.threads || []) {
    index.set(Number(thread.id), { post: thread, threadId: Number(thread.id), isOp: true });
    for (const reply of thread.replies || []) {
      index.set(Number(reply.id), { post: reply, threadId: Number(thread.id), isOp: false });
    }
  }
  return index;
}
...
function quoteHref(boardUri, currentThreadId, target) {
  ...
  return `/${boardUri}/thread/${target.threadId}#p${target.post.id}`;
}
...
module.exports = { buildPostIndex, formatComment, quoteHref };
