...
function timestampMilliseconds(timestamp) {
  if (timestamp === null || timestamp === undefined || String(timestamp).trim() === '') return null;
...
function relativeTime(timestamp, now = Date.now()) {
  const then = timestampMilliseconds(timestamp);
  const current = timestampMilliseconds(now);
  if (then === null || current === null) return 'Unknown time';
